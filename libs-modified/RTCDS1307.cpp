...
bool RTCDS1307::begin() {
  ...
  return !Wire1.endTransmission(1);
}
...
bool RTCDS1307::read(uint8_t address, uint8_t length) {
  ...
  if (write(address, 0)) {
    Wire1.requestFrom(_address, length);
    while (Wire1.available() < length);
    for (uint8_t i = 0; i < length; i++) _buffer[i] = uint8_t(Wire1.read());
    return true;
  }
  return false;
}
...
uint8_t RTCDS1307::wday(uint16_t Y, uint8_t M, uint8_t D) {
  uint8_t m2 = ((uint8_t)28) + (uint8_t)(isLeapYear(Y) ? 1:0);
  uint8_t n[12] = {31, m2, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
...
bool RTCDS1307::write(uint8_t address, uint8_t length) {
  Wire1.beginTransmission(_address);
  Wire1.write(address);
  for (uint8_t i = 0; i < length; i++) Wire1.write(_buffer[i]);
  return !Wire1.endTransmission(1);
}
