 ...
 * press_orig - per-layer: bit 7 = tracking flag, other bits: *_status value at press time.
 *            - On release, bit 7 cleared. */
...
void _layer_handle_pedal_input(struct layer_state & lr, struct layer_state_reg & lr_from_r, int pedal, uint8_t val)
{
	...
	if (pm == PEDAL_MODE_NORMAL) {
		...
		/* Hybrid mode: short press = TOGGLE; long press = PUSH. */
		if (val != 0 && !(pedal_press_orig[i][pedal] & 0x80)) {
			...
			if (elapsed < 800000UL) {
				...
				lr.r.active_status = orig ? 0 : 1;
			} else {
				/* Long press: PUSH (restore to configured active state). */
		...
		if (val != 0 && !(pedal_press_orig[i][pedal] & 0x80)) {
			...
			if (elapsed < 800000UL) {
				/* Short press: TOGGLE (flip from original). */
				lr.r.transposition_extra_status = orig ? 0 : 1;
			} else {
				/* Long press: PUSH (restore to configured active state). */
	...
	for (i = 0; i < PEDALS; i++) {
		...
		if (mode == PEDAL_MODE_NORMAL) {
			if (
				...
				mode == PEDAL_MODE_TOGGLE_PUSH_ACT ||
				mode == PEDAL_MODE_TOGGLE_PUSH_TRANS) {
			set = true;
		}
...
void layer_handle_note_on_special(struct layer_state & lr, uint8_t lnote, uint8_t vol)
{
	...
	lr.notes.ticks_remains[lnote] = 0;
	lr.last_note = lnote;
	...
	if (lr.r.mode & (NOTE_MODE_HOLD | NOTE_MODE_CUT)) {
		...
		lr.notes.ticks_remains[lnote] = ticks + (lr.note_length_mod * (ticks/2)) / 64;
...
void handleSMidiMessage(const midi::Message<128> & msg, uint8_t port)
{
	...
	for (l = 0; l < LAYERS; l++) {
		...
		if (cmd == midi::NoteOff || (cmd == midi::NoteOn && b2 == 0)) {
			...
			if (true || lr.notes.note_origin[b1] & 0x80) {
				lnote = lr.notes.note_origin[b1] & 0x7F;
				note_in_bounds = 1;
			}
		...
		if (
			...
			for (n = 0; n < 128; n++) {
				if (layer_is_playing(lr, n)) {
					if (lr.notes.ticks_remains[n] > 0) {
						lr.notes.ticks_remains[n]--;
					}
					if (lr.notes.ticks_remains[n] == 0) {
...
//								lr.notes.ticks_remains[n+12] = v / 8;
								lr.notes.ticks_remains[n+12] = 20 - v * 15 / 127;
...
#if 0
					if (lr.notes.ticks_remains[n] == 0xFF) {
					} else if (lr.notes.ticks_remains[n] == 1) {
						...
						lr.notes.ticks_remains[n]--;
					} else if (lr.notes.ticks_remains[n] != 0) {
						lr.notes.ticks_remains[n]--;
					}
#endif
				}
...
//			Serial.print(" data "); Serial.print(data);
		...
		for (l = 0; l < LAYERS; l++) {
			...
			if (cmd == midi::NoteOff) {
				...
				if (note_in_bounds && layer_is_playing(lr, lnote)) {
					if (lr.mode & NOTE_MODE_HOLD && lr.notes.ticks_remains[lnote] != 0)
						continue;
					...
					if (lr.cc_pedal3_mode == PEDAL_MODE_NOTELENGTH) {
						if (true || lr.cc_expression < 127) {
							...
							if (lr.mode == NOTE_MODE_HOLD1_4 || lr.mode == NOTE_MODE_CUT1_4) {
							    lr.notes.ticks_remains[lnote] = v / 4;
							} else if (lr.mode == NOTE_MODE_HOLD1_2) {
							    lr.notes.ticks_remains[lnote] = v / 2;
							} else if (lr.mode == NOTE_MODE_SHUFFLE) {
							    lr.notes.ticks_remains[lnote] = v / 8;
							    lr.notes.ticks_remains[lnote] = 4 + v * 15 / 127;
							}
